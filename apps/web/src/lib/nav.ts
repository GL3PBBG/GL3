...
/**
 * Which category each plugin page that declares a `menu` entry belongs to,
 * keyed by pageId. The core tables above deliberately contain no plugin
 * entries: a plugin page's item (label and all) comes from the server's
 * manifest alone, so it exists exactly once and disappears with the plugin —
