 ...
 * genuinely closed, and a real Redis pub/sub round trip (NOTES.md: no
...
describe("WS gateway: routing errors don't crash the process", () => {
  ...
  it("catches a failed `gang` route instead of leaving an unhandled rejection", async () => {
    ...
    gateway = await attachGateway(server, {
      ...
      worldHooks: [], assetDriver: testAssetDriver(), clientIpHeader: config.clientIpHeader, coreHooks: true, hasProperties: false,
    });
