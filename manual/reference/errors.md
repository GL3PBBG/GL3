...
| `insufficient_level` | 409 | crimes, inventory, travel | The player's level is below the gate — the crime, town or firearm requires a higher one (crimes.min_level / locations.min_level / a weapon's minLevel vs player_stats.level; inventory throws it only on a level-model boot, where exp is within-level) |
...
| `rank_too_low` | 409 | inventory | The caller's experience is below the weapon's minRankExp requirement to equip it — exp-model boots only; a level-model boot gates on minLevel and answers insufficient_level instead |
